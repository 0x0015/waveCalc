...
template<typename float_t> void waveChamber2D<float_t>::init(sycl::vec<float_t, dim> s, float_t _dt, unsigned int xPartitions, std::span<const chamberDef_t> chambers, simulationEdgeMode simEdgeMode){
	dt = _dt;
	size = s;
...
template<typename float_t> void waveChamber2D<float_t>::initStateDats(){
...
template<typename float_t> void waveChamber2D<float_t>::initChambers(std::span<const chamberDef_t> chambers){
...
template<typename float_t> void waveChamber2D<float_t>::printuVals(){
...
template<typename float_t> void waveChamber2D<float_t>::writeRawData(){
	const auto& copiedData = state_dats[currentStateNum].copy_to_host();
	using rawWriteRequest_t = rawDataWriter<float_t>::rawWriteRequest;
	rawWriter.createRequest(rawWriteRequest_t{copiedData});
}

template<typename float_t> void waveChamber2D<float_t>::step(){
	...
	handler.parallel_for<class waveChamber2D_step>(sycl::range<2>(partitions.x(), partitions.y()), [=](sycl::id<2> id) {
		array2DWrapper_view<float_t> currentState(currentState_raw, currentState_length, partitions);
		array2DWrapper_view<float_t> previousState(previousState_raw, previousState_length, partitions);
		array2DWrapper<float_t> newState(nextState_raw, nextState_length, partitions);
		...
		switch(edgeMode){
			...
			case VOID:
				...
				if(x < edgeBoarder || x >= partitions.x() - edgeBoarder || y < edgeBoarder || y >= partitions.y() - edgeBoarder){
					newState[{x, y}] = calculateUAtPos2D<float_t>({currentState[{x, y}], x == partitions.x()-1 ? 0 : currentState[{x+1, y}], x == 0 ? 0 : currentState[{x-1, y}], y == partitions.y()-1 ? 0 : currentState[{x, y+1}], y == 0 ? 0 : currentState[{x, y-1}]}, previousState[{x, y}], partitionSize, chamberDefs[chamberNum].c, dt, 10000);
					return;
				}
		}

		newState[{x, y}] = calculateUAtPos2D<float_t>({currentState[{x, y}], currentState[{x+1, y}], currentState[{x-1, y}], currentState[{x, y+1}], currentState[{x, y-1}]}, previousState[{x, y}], partitionSize, chamberDefs[chamberNum].c, dt, chamberDefs[chamberNum].mu);
	});
...
template<typename float_t> void waveChamber2D<float_t>::setSinglePoint(sycl::vec<float_t, 2> point, float_t val){
	array2DWrapper<float_t> state(state_dats[currentStateNum].data(), state_dats[currentStateNum].size(), partitions);
...
template<typename float_t> void waveChamber2D<float_t>::setPointVarryingTimeFunction(sycl::vec<float_t, 2> point, std::function<float_t(float_t)> valWRTTimeGenerator){
	varryingTimeFuncs.push_back({point, valWRTTimeGenerator});
}

template<typename float_t> void waveChamber2D<float_t>::runSimulation(float_t time, float_t printRuntimeStatisticsInterval, float_t saveRawDataInterval){
	...
	if(saveRawDataInterval > 0){
		//saveRawDataInterval is the period(seconds) at which data is saved, so the sampleRate is the inverse of that (Hz)
		rawWriter.createFile("rawOutput.grid2dD", partitions, 1.0/saveRawDataInterval);
...
template class waveChamber2D<float>;
template class waveChamber2D<double>;
