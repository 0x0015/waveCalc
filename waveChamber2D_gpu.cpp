...
template<> void waveChamber2D<EXECUTION_MODE_GPU>::initStateDats(){
...
template<> void waveChamber2D<EXECUTION_MODE_GPU>::initChambers(std::span<const chamberDef_t> chambers){
	...
	auto gpuChambers = agpuUtil::device_vector<chamberDef_t>(cpuChambers);
	chamberDefs = gpuChambers;
}

template<> void waveChamber2D<EXECUTION_MODE_GPU>::printuVals(){
...
template<> void waveChamber2D<EXECUTION_MODE_GPU>::writeToImage(const std::string& filename, double expectedMax){
	agpuUtil::check_error(agpuDeviceSynchronize());
...
template<> void waveChamber2D<EXECUTION_MODE_GPU>::writeRawData(){
	agpuUtil::check_error(agpuDeviceSynchronize());
	const auto& copiedData = state_dats[currentStateNum].copy_to_host();
...
__global__ void step_kernel_2d(double* newState_raw, const double* currentState_raw, const double* previousState_raw, unsigned int stateLength, vec2<unsigned int> partitions, double partitionSize, double dt, const waveChamber2D<EXECUTION_MODE_GPU>::chamberDef_t* chamberDefs, unsigned int chamberDefsLength, simulationEdgeMode edgeMode){
	...
	switch(edgeMode){
		...
		case VOID:
			...
			if(x < edgeBoarder || x >= partitions.x - edgeBoarder || y < edgeBoarder || y >= partitions.y - edgeBoarder){
				newState[{x, y}] = calculateUAtPos2D({x, y}, {currentState[{x, y}], x == partitions.x-1 ? 0 : currentState[{x+1, y}], x == 0 ? 0 : currentState[{x-1, y}], y == partitions.y-1 ? 0 : currentState[{x, y+1}], y == 0 ? 0 : currentState[{x, y-1}]}, previousState[{x, y}], partitionSize, chamberDefs[chamberNum].c, dt, 10000);
				return;
			}
	}

	newState[{x, y}] = calculateUAtPos2D({x, y}, {currentState[{x, y}], currentState[{x+1, y}], currentState[{x-1, y}], currentState[{x, y+1}], currentState[{x, y-1}]}, previousState[{x, y}], partitionSize, chamberDefs[chamberNum].c, dt, chamberDefs[chamberNum].mu);
}

template<> void waveChamber2D<EXECUTION_MODE_GPU>::step(){
	...
	KERNEL_LAUNCH(step_kernel_2d, gpuGridSize, gpuBlockSize)(state_dats[nextStateNum].data(), state_dats[currentStateNum].data(), state_dats[previousStateNum].data(), state_dats.front().size(), partitions, partitionSize, dt, chamberDefs.data(), chamberDefs.size(), edgeMode);

	currentState = nextState;
	currentStateNum = nextStateNum;
}

__global__ void setSinglePoint_kernel_2d(double* state_raw, unsigned int index, double val){
	state_raw[index] = val;
}

template<> void waveChamber2D<EXECUTION_MODE_GPU>::setSinglePoint(vec2<double> point, double val){
	agpuUtil::check_error(agpuDeviceSynchronize());
	array2DWrapper<double> state(state_dats[currentStateNum].data(), state_dats[currentStateNum].size(), partitions);
	KERNEL_LAUNCH(setSinglePoint_kernel_2d, 1, 1)(state_dats[currentStateNum].data(), state.computeIndex((point / partitionSize).convert<unsigned int>()), val);
}

template<> void waveChamber2D<EXECUTION_MODE_GPU>::calculateBestGpuOccupancy(){
	agpuUtil::check_error(agpuOccupancyMaxPotentialBlockSize(&gpuMinGridSize, &gpuBlockSize, step_kernel_2d, 0, 0));
